...
FROM   public.parcels kn WHERE  kn.id <= 50000 AND kn.geom is not null;
...
FROM   public.parcels kn WHERE  kn.id > 50000   AND kn.id <= 100000 AND kn.geom is not null;
...
FROM   public.parcels kn WHERE kn.id > 100000 AND kn.id <= 150000 and  kn.geom is not null ;
...
FROM   public.parcels kn WHERE kn.id > 150000 AND kn.id <= 200000 and  kn.geom is not null ;
...
FROM  public.parcels kn WHERE kn.id > 200000 AND  kn.id <= 300000 AND  kn.geom is not null ;
			END IF;
		END IF;
   	ELSIF in_file_type ='structure'  THEN 
    	...
    	ELSIF in_query_type ='maps' THEN 
			...
			)  FROM public.structures st WHERE st.geom is not null;
   		END IF;
   	END IF;
END;
$BODY$;
